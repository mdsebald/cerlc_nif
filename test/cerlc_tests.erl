...
crc8_sensirion_test() -> crc_test_common(crc8_sensirion, 16#F7).
...
crc_test_common(CrcConfig, ExpectedCrc) ->
  crc_test_common(CrcConfig, ExpectedCrc, ["123", $4, $5, $6, <<"789">>]).

% For testing custom CRC algorithms
crc_test_common({Bits, Polynomial, InitValue, FinalXorValue, Reflected}, ExpectedCrc, TestData) ->
  CrcDefn = cerlc_nif:init(Bits, Polynomial, InitValue, FinalXorValue, Reflected),
  ?assertEqual(ExpectedCrc, cerlc_nif:calc_crc(TestData, CrcDefn));

% For testing predefined CRC algorithms
crc_test_common(CrcConfig, ExpectedCrc, TestData) ->
  CrcDefn = cerlc_nif:init(CrcConfig),
  ?assertEqual(ExpectedCrc, cerlc_nif:calc_crc(TestData, CrcDefn)).
